...
from .ours_operator import LinearOurs, Conv2dOurs, ActOurs, NoActQ, Conv2dFp, LinearFp
...
class QResidualBlockNoBN(nn.Module):
    ...
    def __init__(self, num_feat=64, res_scale=1, pytorch_init=False, qconv='Base', nbits_w=4, nbits_a=4):
        ...
        self.skip_block = nn.Parameter(torch.zeros(1), requires_grad=False)
        self.version = 2
        if qconv == "fp":
            self.version = 0
    ...
    def forward(self, x):
        ...
        if self.version == -1:
            ...
            return identity * self.learnable_shortcut + (out * 1.0)
        elif self.version == 0:
            out = self.conv2(self.relu(self.conv1(x)))
            return identity + out * self.res_scale
        elif self.version == 1:
            if abs(self.learnable_shortcut.item() - 1) < 1e-3:
                return identity
            out = self.conv2(self.relu(self.conv1(x)))
            return identity * self.learnable_shortcut + out * (1 - self.learnable_shortcut)
        elif self.version == 2:
            if abs(self.skip_block.item() - 1) < 1e-3:
                return identity
            out = self.conv2(self.relu(self.conv1(x)))
            return identity * self.learnable_shortcut + out * self.res_scale
        else:
    ...
    def extra_repr(self):
        s_prefix = super(QResidualBlockNoBN, self).extra_repr()
        return '{} (version {})'.format(s_prefix, self.version)
