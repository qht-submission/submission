...
def bool_pass(input):
    ...
    x = (torch.tanh(1*(x-0.5)) / torch.tanh(torch.ones(1).to(x.device)*0.5)) / 2 + 0.5
    out3 = x + input.floor().detach()
    return x_bool.detach() - out3.detach() + out3

def multi_bit_round_pass(weights, scales=None, biases=None):
    if scales is None:
        weight = []
        for i in range(len(weights)):
            weight.append(bool_pass(weights[i]) * (2 ** i))
        weight = sum(weight) # [-15, 15]
        return weight
    else:
        qweight = [0 for _ in range(len(scales))]
        for i in range(len(scales) - 1, -1, -1):
            now_weights = weights - 1
            for j in range(i + 1, len(scales)):
                now_weights -= qweight[j] * scales[j] * (2 ** (j))
            qweight[i] = bool_pass((now_weights * scales[i] - biases[i]))
        for i in range(len(scales)):
            qweight[i] = qweight[i] * (2 ** i)
        qweight = sum(qweight) # [-15, 15]
        return qweight
...
class _Conv2dQ(nn.Conv2d):
    ...
    def extra_repr(self):
        ...
        if hasattr(self, "q_mode"):
            return '{}, {} (q_mode={})'.format(s_prefix, self.kwargs_q, self.q_mode)
...
class Conv2dOurs(_Conv2dQ):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=True, nbits_w=4, nbits_a=4, q_mode=Q_MODE, *kwargs):
        ...
        self.q_mode = q_mode
        self.scales = nn.ParameterList([nn.Parameter(torch.ones(1), requires_grad=True) for _ in range(self.nbits_w)])
        self.biases = nn.ParameterList([nn.Parameter(torch.zeros(1), requires_grad=True) for _ in range(self.nbits_w)])
        print(f"*** conv2d quant method ***: {self.q_mode}")
    ...
    def forward(self, x):
        ...
        alpha = grad_scale(self.alpha, g)
        w_q = ((multi_bit_round_pass(self.weight / alpha * 2, self.scales, self.biases) - 1) / 2 + 1) * alpha

        res = F.conv2d(x, w_q, self.bias, self.stride, self.padding, self.dilation, self.groups)
        return res

# RBD for Linear
class LinearOurs(_LinearQ):
    def __init__(self, in_features, out_features, bias=True, nbits_w=4, nbits_a=4, q_mode=Q_MODE, **kwargs):
        ...
        self.biases = nn.ParameterList([nn.Parameter(torch.zeros(1), requires_grad=True) for _ in range(self.nbits_w)])
        self.q_mode = q_mode
        print(f"*** linear quant method ***: {self.q_mode}")
