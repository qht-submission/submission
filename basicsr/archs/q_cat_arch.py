...
from basicsr.archs.arch_util import QConv2ds, QLinears, QActQs
...
class DynamicPosBias(nn.Module):
    ...
    """
    def __init__(self, dim, num_heads, residual, qconv='FP32', nbits_w=4, nbits_a=4):
        super().__init__()
        self.residual = residual
        self.num_heads = num_heads
        self.pos_dim = dim // 4
        self.pos_proj = QLinears[qconv](2, self.pos_dim, nbits_w=nbits_w, nbits_a=nbits_a)
        self.pos1 = nn.Sequential(
            nn.LayerNorm(self.pos_dim),
            nn.ReLU(inplace=True),
            QLinears[qconv](self.pos_dim, self.pos_dim, nbits_w=nbits_w, nbits_a=nbits_a)
        )
        ...
        self.pos3 = nn.Sequential(
            nn.LayerNorm(self.pos_dim),
            nn.ReLU(inplace=True),
            QLinears[qconv](self.pos_dim, self.num_heads, nbits_w=nbits_w, nbits_a=nbits_a)
        )
...
class Attention_axial(nn.Module):
    ...
    """
    def __init__(self, dim, resolution, idx, split_size=7, dim_out=None, num_heads=6, attn_drop=0., proj_drop=0., qk_scale=None, position_bias=True, qconv='FP32', nbits_w=4, nbits_a=4):
        super().__init__()
        self.dim = dim
        ...
        if self.position_bias:
            self.pos = DynamicPosBias(self.dim // 4, self.num_heads, residual=False, qconv=qconv, nbits_w=nbits_w, nbits_a=nbits_a)
...
class Attention_regular(nn.Module):
    ...
    Args:
        ...
        position_bias (bool): The dynamic relative position bias. Default: True
    """
    def __init__(self, dim, resolution, idx, split_size=[2,4], dim_out=None, num_heads=6, attn_drop=0., proj_drop=0., qk_scale=None, position_bias=True, qconv='FP32', nbits_w=4, nbits_a=4):
        super().__init__()
        self.dim = dim
        self.dim_out = dim_out or dim
        self.resolution = resolution
...
class CATB_axial(nn.Module):
    ...
    def __init__(self, dim, reso, num_heads,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, qconv='FP32', nbits_w=4, nbits_a=4):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        self.proj = QLinears[qconv](dim, dim)
        ...
        self.attns = nn.ModuleList([
                    Attention_axial(
                        ...
                        qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, position_bias=True, qconv=qconv, 
                        nbits_w=nbits_w, nbits_a=nbits_a)
                    for i in range(self.branch_num)])
            
        mlp_hidden_dim = int(dim * mlp_ratio)

        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, out_features=dim, act_layer=act_layer, drop=drop, qconv=qconv, nbits_w=nbits_w, nbits_a=nbits_a)
        self.norm2 = norm_layer(dim)

        self.get_v = QConv2ds[qconv](dim, dim, kernel_size=3, stride=1, padding=1, groups=dim, nbits_w=nbits_w, nbits_a=nbits_a) # DW Conv
...
class QResidualGroup(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        if self.version == -1:
            ...
            x = res * self.learnable_shortcut + x
        else:
            if self.skip[self.version]:
                x = res * self.learnable_shortcut
            else:
                for blk in self.blocks:
                    if self.use_chk:
                        x = checkpoint.checkpoint(blk, x, x_size)
