...
class SRModel(BaseModel):
    ...
    def optimize_parameters_kd(self, current_iter, features_in_hook, features_in_hook_teacher, skip_blocks=[]):
        self.optimizer_g.zero_grad()
        ...
        for index in range(len(features_in_hook)):
            if index in skip_blocks or index - 1 in skip_blocks:
                continue
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img, self_ensemble=False):
        ...
        if not hasattr(self, "all_metric_results"):
            self.all_metric_results = OrderedDict()
        if dataset_name not in self.all_metric_results:
            self.all_metric_results[dataset_name] = []
        if with_metrics:
        ...
        if with_metrics:
            for metric in self.metric_results.keys():
                ...
                self.all_metric_results[dataset_name].append(self.metric_results)
